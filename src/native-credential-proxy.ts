 ...
 *
 * Checks `process.env` first (set when the service is launched with the var
 * exported), then falls back to reading `.env` directly via core's
 * `readEnvFile` — the host does not load `.env` into `process.env`, so the
 * file fallback is what makes the flag work for a plain `pnpm run dev`.
 */
export function nativeCredentialsEnabled(): boolean {
  if (process.env[NATIVE_CREDENTIALS_FLAG] === 'true') return true;
  const fromEnvFile = readEnvFile([NATIVE_CREDENTIALS_FLAG]);
  return fromEnvFile[NATIVE_CREDENTIALS_FLAG] === 'true';
}
